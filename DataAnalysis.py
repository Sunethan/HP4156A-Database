...
class Read(object):
    def __init__(self, data, format, xlabel=None, ylabel=None, x=None, y=None, floatKey='Yes'):
        if data is not None:
            self.__data = data
            if data.split(".")[1] == 'csv':
                if format == 'Standard':
                    ...
                    with open(self.__data, newline='', encoding='utf-8-sig') as csv_file:
                        ...
                        for _index, row in enumerate(rows):
                            if row['DataName'] == 'DataValue':
                                self.X.append(float(row['V1']))
                                self.Y.append(float(row['I11']))
                    self.X = np.asarray(self.X)
                    self.Y = np.asarray(self.Y)
            elif data.split(".")[1] == 'TXT':
                if format == 'HP4156':
                    self.X, self.Y = utils.parse_text(self.__data, 'V', 'A')
                    self.Vm = None
                    self.Im = None
            else:
...
class DopingProfile(CSV):
    ...
    def convert_plx(self, add='No', info=None):
        ...
        # 儲存檔案
        new_format = self.get_location().replace("raw_data", "new_data").replace(".csv", ".plx")
        utils.save(new_format, 'X', 'Y', new_x, new_y, form='', title='DopingConcentration')
